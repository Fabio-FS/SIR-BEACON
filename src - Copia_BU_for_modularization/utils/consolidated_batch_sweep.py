...
def consolidated_batch_sweep(
    ...
    """Unified batch sweep function for all model types"""
    ...
    # Handle different state shapes based on the shape of the first result
    first_state = results[0][0]
    if isinstance(first_state, tuple) and len(first_state) == 4:  # SIRV model
        S = jnp.concatenate([res[0] for res in results])
        I = jnp.concatenate([res[1] for res in results])
        R = jnp.concatenate([res[2] for res in results])
        V = jnp.concatenate([res[3] for res in results])
        final_states = (S, I, R, V)
    else:  # SIR, SIRT, SIRM models
        S = jnp.concatenate([res[0] for res in results])
        I = jnp.concatenate([res[1] for res in results])
        R = jnp.concatenate([res[2] for res in results])
        final_states = (S, I, R)
